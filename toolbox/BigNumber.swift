...
struct BigUInt {
    ...
    internal mutating func add(_ b: BigUInt, shiftedBy shift: Int = 0) {
        ...
        var carry = false
        var bi = 0
        let bc = b.storage.count
        while bi < bc || carry {
            let ai = shift + bi
            let (d, c) = storage[ai].addingReportingOverflow(b.storage[bi])
            if carry {
...
extension BigUInt {
    ...
    public mutating func multiply(byWord y: Word) {
        ...
        let c = self.count
        for i in 0 ..< c {
            let (h, l) = storage[i].multipliedFullWidth(by: y)
    ...
    public static func *(x: BigUInt, y: BigUInt) -> BigUInt {
        ...
        if yc == 1 { return x.multiplied(byWord: y.storage[0]) }
        if xc == 1 { return y.multiplied(byWord: x.storage[0]) }
        ...
        if Swift.min(xc, yc) <= BigUInt.directMultiplicationLimit {
            ...
            for i in (0 ..< right.count).reversed() {
                result.multiplyAndAdd(left, right.storage[i], shiftedBy: i)
            }
            return result
        }
...
func + (_ lhs: BigUInt, _ rhs: BigUInt) -> BigUInt {
    var lhsTemp = lhs
    lhsTemp.add(rhs)
    return lhsTemp
}
